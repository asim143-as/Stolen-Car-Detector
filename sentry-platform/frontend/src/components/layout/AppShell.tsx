...
export default function AppShell({
  ...
  useEffect(() => {
    loadUserCachedData()
    // Listen for storage events when user updates profile in Settings
    ...
    <div className="flex min-h-screen bg-slate-50/70 dark:bg-slate-950">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-64 shadow-2xl">{SidebarInner}</aside>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-200/80 dark:border-slate-800/80 bg-white/85 dark:bg-slate-900/85 backdrop-blur-md px-4 sm:px-8 py-3">
          {/* Left: Search input (or mobile menu button) */}
          ...
          {/* Right Header Actions (Bell notification & Avatar) */}
